...
std::vector<pokemon_generator*> pokemonReplacer;
...
void io_init_terminal()
    {
      ...
      init_pair(COLOR_BLACK, COLOR_BLACK, COLOR_MAGENTA);
          ...
          if(uInput == '5' || uInput == ' ' || uInput == '.'){
            ...
            if(pcXcoord > 1 && costOfTerrain((pcXcoord) -1,pcYcoord,w[x][y]->grid,3) != INT_MAX){
              ...
              continue;
                ...
                mvprintw(1,0,"Your pokebucks went from %d to %d! You can spend this in the PokeMart!", tempMoney, pokeBucks);
                mvprintw(2,0,"Press 'c' to continue with your journey!");
                uInput = 'v';
                while(uInput != 'c'){
                  uInput = getch();
                }
            ...
            if(w[x][y]->grid[pcXcoord][pcYcoord] == 3){//Pokemart
              clear();
              refresh();
              mvprintw(0,0,"In a pokeMart! You have %d potions, %d pokeballs and a balance of %d pokebucks!", bagPotion, bagPokeballs, pokeBucks);
              // mvprintw(1,0,"Replenishing your bag!");
              // bagPokeballs = 3;
              // bagPotion = 2;
              bool broke = false;
              while(pokeBucks >= 100 && broke == false){
                mvprintw(1,0,"Would you like to purchase more pokeballs 'b' or potions 'p'");
                while(uInput != 'b' && uInput != 'p'){
                  uInput = getch();
                }
                if(uInput == 'p'){
                  clear();
                  ...
                  mvprintw(0,0,"Adding a potion to your bag. You now have %d potions with %d pokebucks", bagPotion, pokeBucks);
                }
                else if(uInput == 'b'){
                  clear();
                  refresh();
                  ...
                  pokeBucks = pokeBucks - 100;
                  mvprintw(0,0,"Adding a pokeball to your bag. You now have %d pokeballs with %d pokebucks", bagPokeballs, pokeBucks);
                }
              }
              
              mvprintw(1,0,"Not enough pokebucks! You only have %d pokebucks, go defeat more trainers!", pokeBucks);
              mvprintw(2,0,"Currently have %d potions and %d pokeballs", bagPotion, bagPokeballs);
              refresh();
              while(uInput != '<'){
                uInput = getch();
                if(uInput == '<'){
              ...
              if(pokemonReplacer.size() == 0){
                ...
                refresh();
                mvprintw(2,0,"Would you like to switch a pokemon from here to be one of the possible 6 you have? Type 'y' or 'n'");
                while(uInput != 'y' && uInput != 'n'){
                  uInput = getch();
                }
                if(uInput == 'y'){//output your pokemon first, then the chosen ones
                  clear();
                  refresh();
                  int j = 1;
                  mvprintw(0,0,"Here is the list of your 6 pokemon");
                  for(int i =0; i<6; i++){
                    if(w[x][y]->characterTable[pcXcoord][pcYcoord]->pokemonList[i] != NULL){
                      mvprintw(j,0,"%d Pokemon %s : level %d", i, w[x][y]->characterTable[pcXcoord][pcYcoord]->pokemonList[i]->pokemonIdentifier,w[x][y]->characterTable[pcXcoord][pcYcoord]->pokemonList[i]->level);
                      j++;
                    }
                  }
                  mvprintw(j+1, 0, "Enter the number of the pokemon you would like to get rid of: ");
                  uInput = getch();
                  int userRid = int(uInput - '0');
                  pokemon_generator *temp2 = w[x][y]->characterTable[pcXcoord][pcYcoord]->pokemonList[userRid];
                  delete temp2;
                  w[x][y]->characterTable[pcXcoord][pcYcoord]->pokemonList[userRid] = NULL;

                  j =1;
                  clear();
                  refresh();
                  mvprintw(0,0,"Here is a list of pokemon's you can choose from:");
                  for(int i =0; i< pokemonReplacer.size(); i++){
                    mvprintw(j,0,"%d Pokemon %s : level %d", i, pokemonReplacer.at(i)->pokemonIdentifier, pokemonReplacer.at(i)->level);
                    j++;
                  }
                  mvprintw(j+1, 0, "Enter the number of the pokemon you would like to add to your collection: ");
                  uInput = getch();
                  int userChoice = int(uInput - '0');
                  pokemon_generator *temp = pokemonReplacer.at(userChoice);
                  // delete pokemonReplacer.at(userChoice);
                  // pokemonReplacer.erase(pokemonReplacer.begin() + userChoice);
                  // if(pokemonReplacer.size() == 0){
                  //   pokemonReplacer.clear();
                  // }

                  w[x][y]->characterTable[pcXcoord][pcYcoord]->pokemonList[userRid] = temp;
                  pokemonReplacer.erase(pokemonReplacer.begin() + userChoice);
                  if(pokemonReplacer.size() == 0){
                    pokemonReplacer.clear();
                  }
...
void battleWildPokemon(Characters *pc, pokemon_generator *wildPokemon){
  ...
  if(wildPokemon->currentHp == 0 && pokemonReplacer.size() > 10){
    delete wildPokemon;
    uInput = 'v';
    clear();
    refresh();
  ...
  else if(wildPokemon->currentHp == 0 && pokemonReplacer.size() < 10){
    wildPokemon->currentHp = wildPokemon->hp;
    pokemonReplacer.push_back(wildPokemon);
    uInput = 'v';
    clear();
    refresh();
    mvprintw(1,0,"Enemy Pokemon has been defeated! Visit a pokecenter to view and stash this pokemon! Press 'c' to continue with your journey!");
    while(uInput != 'c'){
      uInput = getch();
    }
  }
